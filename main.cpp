...

	{
		...
		while ( caracter != 13 || caracter != 27 ) {
			...
			if ( caracter == 13 ) {         //enter
				...
				int lungime = 0 , lungime_modificat = 0;
				char sir_auxiliar[MAX_VEC] = { 0 } , sir_complet[MAX_VEC] = { 0 } , sir_auxiliar2[MAX_VEC] = { 0 };
				settextstyle (text_style , HORIZ_DIR , text_size2);

				evaluare (String);		//verific corectitudinea
				citire_Big (String , vector1_bigint , lungime);
				lungime_modificat = lungime;

				infix_to_postfix_Big (vector1_bigint , vector2_bigint , lungime_modificat);
				numar1_bigint = eval_postfix_Big (vector2_bigint , lungime_modificat);
...

	{
		...
		while ( caracter != 13 || caracter != 27 ) {
			...
			if ( caracter == 13 ) {         //enter
				...
				infix_to_postfix_Real (vector1_real , vector2_real , lungime_modificat);
				rezultat_real = eval_postfix_Real (vector2_real , lungime_modificat);
		...
		while ( 1 && not_escaped ) {		//daca sa afisat si nu a fost dat escape
			if ( ismouseclick (WM_LBUTTONDOWN) ) {
				clearmouseclick (WM_LBUTTONDOWN);
				x = mousex ();
				y = mousey ();
...
void meniu2 () {
	short x , y;
	setbkcolor (0); // fundal negru pentru scris
	cleardevice ();
	settextstyle (text_style , HORIZ_DIR , text_size1);
	setcolor (15);
...
// Adauga cifra x la stanga numarului (nefolosit
